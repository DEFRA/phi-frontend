...
```bash
docker build --target development --no-cache --tag phi-frontend:development .
```

Run:

```bash
docker run -p 3000:3000 phi-frontend:development
```
...
```bash
docker build --no-cache --tag phi-frontend .
```
