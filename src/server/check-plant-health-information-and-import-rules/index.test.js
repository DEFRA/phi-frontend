...
jest.mock('~/src/server/check-plant-health-information-and-import-rules/problem-with-service/controller', () => ({
...
describe('problemWithService plugin', () => {
 ...
 test('should register the /check-plant-health-information-and-import-rules/problem-with-service route', () => {
    ...
    const problemWithServiceRoute = routes.find(
      (routes) => routes.path === '/check-plant-health-information-and-import-rules/problem-with-service' && routes.method === 'get'
    )
    expect(problemWithServiceRoute).toBeDefined()
    expect(problemWithServiceRoute.settings.handler).toBe(problemWithServiceController.handler)
  });
});
